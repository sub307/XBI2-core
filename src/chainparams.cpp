...
void CChainParams::UpdateLLMQTestParams(int size, int threshold) {
    auto& params = consensus.llmqs.at(Consensus::LLMQ_5_60);
    params.size = size;
...
public:
    CMainParams() {
        ...
        vSporkAddresses = {"Gbi17esbFNFrmcniL8eMBcCWJtRb9xdhmR"};
        nMinSporkKeys = 1;
...
public:
    CDMNTestParams() {
        ...
        nBIP44IDCurrent = 0x80000213;
        nModifierInterval = 10 * 60;    // 10 minutes
        nStakeMinConfirmations = 25;    // 225 * 2 minutes
        nTargetSpacing = 45;            // 45 seconds
        nTargetTimespan = 6 * 60;       // 6 mins
        ...
        nLLMQConnectionRetryTimeout = 60;
        nFulfilledRequestExpireTime = 60*60;
        vSporkAddresses = {"GSL2ghr6mG4z2eLQnuKNbMM9CebNdUkCrs"};
        nMinSporkKeys = 1;
        fBIP9CheckMasternodesUpgraded = true;
