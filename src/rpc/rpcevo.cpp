...
// Allows to specify Ghost address or priv key. In case of Ghost address, the priv key is taken from the wallet
static CKey ParsePrivKey(CHDWallet *pwallet, const std::string &strKeyOrAddress, bool allowAddresses = true) {
...
static CKeyID ParsePubKeyIDFromAddress(CHDWallet *pwallet, const std::string& strAddress, const std::string& paramName)
{
...
static void FundSpecialTx(CHDWallet *pwallet, CMutableTransaction& tx, const SpecialTxPayload& payload, const CTxDestination& fundDest)
{
...
static std::string SignAndSendSpecialTx(const CMutableTransaction& tx, const JSONRPCRequest& request)
{
    {
    LOCK(cs_main);
...
void protx_register_fund_help(CHDWallet *pwallet)
{
    throw std::runtime_error(
            ...
            "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 1000 Ghost\n"
...
void protx_register_help(CHDWallet *pwallet)
{
    throw std::runtime_error(
...
void protx_register_submit_help(CHDWallet *pwallet)
{
    throw std::runtime_error(
...
UniValue protx_register(const JSONRPCRequest& request)
{
    ...
    if (!request.params[paramIdx + 6].isNull()) {
        ...
        if (!IsValidDestination(fundDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Ghost address: ") + request.params[paramIdx + 6].get_str());
    }
    ...
    if (isFundRegister) {
        ...
        for (uint32_t i = 0; i < tx.vpout.size(); i++) {
            if (tx.vpout[i]->GetValue() == Params().GetConsensus().nMasternodeCollateral) {
                collateralIndex = i;
                break;
            }
        }
        ...
        SetTxPayload(tx, ptx);
        return SignAndSendSpecialTx(tx, request);
    } else {
...
void protx_update_service_help(CHDWallet *pwallet)
{
    throw std::runtime_error(
...
UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    CMutableTransaction tx;
    tx.SetType(TRANSACTION_PROVIDER_UPDATE_SERVICE);
...
void protx_update_registrar_help(CHDWallet *pwallet)
{
    throw std::runtime_error(
...
UniValue protx_update_registrar(const JSONRPCRequest& request)
{
    ...
    CMutableTransaction tx;
    tx.SetType(TRANSACTION_PROVIDER_UPDATE_REGISTRAR);
...
void protx_revoke_help(CHDWallet *pwallet)
{
    throw std::runtime_error(
...
UniValue protx_revoke(const JSONRPCRequest& request)
{
    ...
    CMutableTransaction tx;
    tx.SetType(TRANSACTION_PROVIDER_UPDATE_REVOKE);

    if (!request.params[4].isNull()) {
        CTxDestination feeSourceDest = DecodeDestination(request.params[4].get_str());
        if (!IsValidDestination(feeSourceDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Ghost address: ") + request.params[4].get_str());
        FundSpecialTx(pwallet, tx, ptx, feeSourceDest);
...
static bool CheckWalletOwnsKey(CHDWallet *pwallet, const CKeyID& keyID) {
...
static bool CheckWalletOwnsScript(CHDWallet *pwallet, const CScript& script) {
#ifndef ENABLE_WALLET
    return false;
#else
    if (!pwallet) {
...
UniValue BuildDMNListEntry(CHDWallet *pwallet, const CDeterministicMNCPtr& dmn, bool detailed)
{
    if (!detailed) {
...
UniValue protx_list(const JSONRPCRequest& request)
{
    ...
    CHDWallet *const pwallet = GetParticlWallet(wallet.get());
#else
    CHDWallet *const pwallet = nullptr;
#endif
